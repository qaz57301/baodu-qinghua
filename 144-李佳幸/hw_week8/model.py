...
class TripletNetwork(nn.Module):
    ...
    def forward(self, sa, sp = None, sn = None, margin = None):
        if sp is not None:
            va = self.sentence_encoder(sa)
            vp = self.sentence_encoder(sp)
            vn = self.sentence_encoder(sn)
            return self.cosine_triplet_loss(va, vp, vn, margin)
        else:
            return self.sentence_encoder(sa)
