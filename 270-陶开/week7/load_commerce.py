...
class DataGenerator:
    ...
    def load(self):
        self.data = []
        commerce_comment = pd.read_csv("文本分类练习.csv")
        for i in range(len(commerce_comment)):
            tag = commerce_comment["label"][i]
            label = self.label_to_index[tag]
            title = commerce_comment["review"][i]
            if self.config["model_type"] == "bert":
...
if __name__ == "__main__":
    ...
    dl = load_data("文本分类练习.csv", config=Config, shuffle=True)
    # train_data = []
